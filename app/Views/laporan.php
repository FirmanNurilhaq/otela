...
<main class="max-w-screen-xl mx-auto px-4 py-10">
  ...
  <div class="bg-white p-4 rounded-lg shadow-md mb-6 flex flex-wrap items-center justify-between gap-4">
    <form action="<?= base_url('laporan') ?>" method="get" class="flex flex-wrap items-center gap-4">
      ...
      <div>
        <label for="modal" class="block text-sm font-medium text-gray-700">Modal Usaha (Rp)</label>
        <input type="number" id="modal" name="modal" value="<?= esc($filter_modal) ?>" placeholder="Contoh: 500000"
          class="mt-1 w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent text-sm" />
      </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
    <div class="bg-white p-6 rounded-lg shadow-md text-center">
      ...
      <p id="displayPendapatan" class="text-3xl font-bold text-teal-600 mt-2">Rp <?= number_format($total_pendapatan, 0, ',', '.') ?></p>
    </div>
    <div class="bg-white p-6 rounded-lg shadow-md text-center">
      <h3 class="text-lg font-semibold text-gray-500">Modal Usaha</h3>
      <p id="displayModal" class="text-3xl font-bold text-blue-600 mt-2">Rp 0</p>
    </div>
    <div class="bg-white p-6 rounded-lg shadow-md text-center">
      <h3 class="text-lg font-semibold text-gray-500">Keuntungan / Kerugian</h3>
      <p id="displayUntungRugi" class="text-3xl font-bold text-gray-700 mt-2">Rp 0</p>
    </div>
  </div>
...
<div id="printModal" class="fixed inset-0 bg-black bg-opacity-50 hidden justify-center items-center z-50">
  <div class="bg-white p-6 rounded-lg shadow-md w-full max-w-sm">
    ...
    <p class="text-sm text-gray-600 mb-4">Pastikan nilai modal di halaman utama sudah diisi sebelum mencetak.</p>
    <div class="mb-4">
      <label class="block mb-1">Bulan dan Tahun Laporan:</label>
      <input type="month" id="bulanCetak" class="w-full border p-2 rounded" value="<?= esc($filter_bulan) ?>" readonly />
    </div>
    <div class="flex justify-end gap-2">
      <button onclick="toggleModal(false)" class="px-4 py-2 bg-gray-300 rounded">Batal</button>
      <button onclick="submitCetak()" class="px-4 py-2 bg-teal-600 text-white rounded">Lanjutkan & Cetak</button>
    </div>
  </div>
</div>
...
<script>
  function toggleModal(show) {
    if (show) {
      document.getElementById('bulanCetak').value = document.getElementById('bulan').value;
    }
  ...
  function submitCetak() {
    ...
    const url = `<?= base_url('laporan/print?bulan=') ?>${bulan}&modal=${modal}`;
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const displayUntungRugi = document.getElementById('displayUntungRugi');
    ...
    function hitungUntungRugi() {
      ...
      const untungRugi = totalPendapatan - modal;

      displayModal.textContent = formatRupiah(modal);
      displayUntungRugi.textContent = formatRupiah(untungRugi);

      displayUntungRugi.classList.remove('text-green-600', 'text-red-600', 'text-gray-700');
      if (untungRugi > 0) {
        displayUntungRugi.classList.add('text-green-600');
      } else if (untungRugi < 0) {
        displayUntungRugi.classList.add('text-red-600');
      } else {
        displayUntungRugi.classList.add('text-gray-700');
      }
    }
    inputModal.addEventListener('input', hitungUntungRugi);
    hitungUntungRugi();
  });
